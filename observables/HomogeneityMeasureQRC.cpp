...
#include "HomogeneityMeasureQRC.hpp"
#include <chrono>

void HomogeneityQRC::process() {
	...
	for (int i = 0; i < epsilons.size(); i++) {
        ...
        std::vector<double> cnumbers = QRCBall(origin, epsilon,vmax); // calcula la distancia promedio entre puntos de una esfera con otra que se superpone, ambas con radio epsilon
...
std::vector<double> HomogeneityQRC::QRCBall(Vertex::Label p1, int epsilon,int vmax) {
    std::vector<double> QRCnumbers;
    std::vector<Vertex::Label> bola=ball(p1, epsilon);
    for(auto p2 : bola){
        QRCnumbers.push_back(averageSphereDistanceForHomogeneity(p2,epsilon,vmax));
    }
    return QRCnumbers; // array with all the coordination numbers of the vertices inside a ball centered at p1 with radius epsilon
}

double HomogeneityQRC::averageSphereDistanceForHomogeneity(Vertex::Label p1, int epsilon,int vmax) {
