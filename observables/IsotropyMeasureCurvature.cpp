...
#include "IsotropyMeasureCurvature.hpp"
#include <chrono>

void IsotropyCurvature::process() {
    std::vector<std::vector<double>> epsilonDistanceList;
	...
	for (int i = 0; i < epsilons.size(); i++) {
        ...
        std::vector<double> averageDistance = DirectionalAverageSphereDistance(origin, epsilon,vmax); // calcula la distancia promedio entre puntos de una esfera con otra que se superpone, ambas con radio epsilon
    ...
    std::string tmp = "";
    for (auto radiusarray : epsilonDistanceList) { // va a escribir de arriba a abajo, en radio creciente
            for (double dst : radiusarray){ // para cada radio, pone uno al lado del otro los valores de distancias promedio en cada una de las direcciones
...
std::vector<double> IsotropyCurvature::DirectionalAverageSphereDistance(Vertex::Label p1, int epsilon,int vmaxx) {

    std::vector<double> directionaldistances(sphere(p1, epsilon).size());
    int k=0;
    for(auto p2 : sphere(p1, epsilon)){
        doneL.resize(vmaxx + 1, false); // la funcion resize agrega vmax + 1 elementos con valor false al vector. Si ya tenia menos elementos que vmax+1 elementos los agrega al final, y si tenia mas, borra todos los que sobran a partir de vmax mas 1, y pone los que quiere al principio
        vertexL.resize(vmaxx + 1, false);
        ...
        directionaldistances[k]=averageDistance;
        k=k+1;
    }
