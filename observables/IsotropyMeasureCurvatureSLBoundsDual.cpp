...
#include "IsotropyMeasureCurvatureSLBoundsDual.hpp"
#include <chrono>
#include <cstdlib>

void IsotropyCurvatureSLBoundsDual::process() {
...
std::vector<std::vector<double>> IsotropyCurvatureSLBoundsDual::DirectionalAverageSphereDistanceDual(Triangle::Label p1, int epsilon,int vmaxx) {
    ...
    for(auto p2 : sphere1){
        ...
        if(t1==t2 || abs(t1-t2)==1){
            SpaceLikedirectionaldistances.push_back(averageDistance);
        }
        if((abs(t1-t2)==epsilon) || (abs(t1-t2)==epsilon+1) || (abs(t1-t2)==epsilon-1) || (abs(t2-t1)==T-epsilon) || (abs(t2-t1)==T-epsilon+1)  || (abs(t2-t1)==T-epsilon-1)){
