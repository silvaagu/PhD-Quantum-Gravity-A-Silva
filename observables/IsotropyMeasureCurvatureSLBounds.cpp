#ifndef ISOTROPYMEASURECURVATURESLBOUNDS_CPP_INCLUDED
#define ISOTROPYMEASURECURVATURESLBOUNDS_CPP_INCLUDED

#include <string>
#include <vector>
#include <algorithm>
#include <unordered_map>
#include "IsotropyMeasureCurvatureSLBounds.hpp"
#include <chrono>
#include <cstdlib>

void IsotropyCurvatureSLBounds::process() {
	...
	for (int i = 0; i < epsilons.size(); i++) {
        ...
        std::vector<std::vector<double>> averageDistance = DirectionalAverageSphereDistance(origin, epsilon,vmax); // calcula la distancia promedio entre puntos de una esfera con otra que se superpone, ambas con radio epsilon
    ...
    std::string tmp = "";
    int R=epsilonDistanceList.size(); //cantidad de radios
    int T=epsilonDistanceList[0].size(); // cantidad de tipos
...
std::vector<std::vector<double>> IsotropyCurvatureSLBounds::DirectionalAverageSphereDistance(Vertex::Label p1, int epsilon,int vmaxx) {
    ...
    std::vector<Vertex::Label> sphere1=sphere(p1, epsilon);
    for(auto p2 : sphere1){
        std::vector<bool> doneL(vmaxx + 1, false); //array de verdaderos o falsos
        std::vector<bool> vertexL(vmaxx + 1, false);
...
#endif // ISOTROPYMEASURECURVATURESLBOUNDS_CPP_INCLUDED
