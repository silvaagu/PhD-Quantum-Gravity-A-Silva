...
void Universe::exportGeometry(std::string fname) {
	...
	intTMap.resize(triangles.size()); // le da el tamaño de la cantidad de triangulos que tiene la triangulacion
	...
	output += std::to_string(vertices.size()); // pone la cantidad de vèrtices como el principio del string
	output += "\n"; // salto de linea

	for (int j = 0; j < intVMap.size(); j++) {
		output += std::to_string(intVMap.at(j)->time); // llena el tiempo de cada vèrtice (del 0 al verticices.size-1)
		output += "\n"; // todos separados con un salto de linea
	}

	output += std::to_string(vertices.size()); // vuelve a poner la cantidad de vèrtices
	output += "\n"; // salto de linea
	...
	for (int j = 0; j < intTMap.size(); j++) { // hace un for por cada uno de los triangulos en la triangulacion
		Triangle::Label t = intTMap.at(j); // pone en t un puntero a cada triangulo que está en el mapa
		...
		Vertex::Label tVs [3] = {t->getVertexLeft(),t->getVertexRight(),t->getVertexCenter()}; // array que guarda los tres vértices de cada triangulo
		for (auto v : tVs) { // pone cada uno de los vértices asociados a cada triangulo se parados por un /n: izquierdo /n derecho /n central /n
	...
	std::cout << fname << "\n"; // una vez que termina de exportar todo la geometría, escribe en donde la guardó
}
